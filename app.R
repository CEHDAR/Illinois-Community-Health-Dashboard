                     ...
                     fluidRow( 
                       column(
                         ...
                         p(
                           ...
                           p(
                             "A dashboard is", tags$span(style = "color: #0077b6; font-weight: 600;", "web-based application"), "that presents customized data visualizations and geographic maps conveying", HTML('<span style="color: #0077b6; font-weight: 600;">place-based information</span>.'),"Dashboards allow for interactivity with its users and have the flexibility to present multiple pieces of information within and across multiple tabs in a single dashboard."),
                           ...
                           tags$ul(
                             ...
                             tags$li(strong("Health Conditions"), " (asthma, cancer, coronary heart disease, diabetes, poor mental health)"),
...
## Tab 5c: Motivation & Development ----
tab5c_ui <- tabPanel("Motivation and Development",
                     fluidRow( 
                       column(
                         width = 10,
                         offset = 1,
...
tab5d_ui <- tabPanel("Web App Information",
                     fluidRow( 
                       column(
                         ...
                         h4(icon("info"), "Web App Information"),
                         ...
                         p(style = "text-align: center;", "Gounder, B. & Cho, S. (2026).", tags$em("CEHDAR and the Illinois Community Health Dashboard: Communicating Environmental and Public Health Data with Interactive Geospatial Maps for Community Access."), "Zenodo.", tags$a(href = "https://doi.org/10.5281/zenodo.19441572", target = "_blank", "https://doi.org/10.5281/zenodo.19441572")),
                         hr(),
                         
                         tags$p(style = "font-size: 13px; color: gray; margin-top: 20px;",
                                "This site uses Google Analytics, a web analytics service provided by Google, to understand how visitors use this dashboard (e.g., number of visits and time spent). ",
                                "This information does not identify individual users nor is it used for advertising. ",
                                "You can prevent this data collection by installing the ",
                                tags$a(href = "https://tools.google.com/dlpage/gaoptout", target = "_blank", style = "color: gray; text-decoration: underline;", "Google Analytics opt-out add-on."),
                                "To learn more, visit ",
                                tags$a(href = "https://policies.google.com/technologies/partner-sites", target = "_blank", style = "color: gray; text-decoration: underline;", "Google's Privacy & Terms.")),
...
tab1_server <- function(input, output, session) {
  ...
  output$populationChart <- renderPlotly({
    ...
    plot_ly(data = as.data.frame(population_data), x = ~Population, y = ~as.numeric(Percentage), type = 'bar', color = ~Population, colors = "Dark2",
      ...
      layout(title = list(text = paste("Population Demographics -", input$neighborhood), x = 0.5, xanchor = "center"),
...
tab3_server <- function(input, output, session) {
  ...
  output$climateChart <- renderPlotly({
    ...
    plot_ly(data = as.data.frame(climate_data), x = ~Category, y = ~as.numeric(Percentage), type = 'bar', color = ~Category, colors = "Dark2",
      ...
      layout(title = list(text = paste("Climate Events -", input$neighborhood_climate), x = 0.5, xanchor = "center"),
...
tab4_server <- function(input, output, session) {
  ...
  output$healthChart <- renderPlotly({
    ...
    plot_ly(data = as.data.frame(health_data), x = ~Category, y = ~as.numeric(Percentage), type = 'bar', color = ~Category, colors = "Dark2",
      ...
      layout(title = list(text = paste("Health Conditions -", input$neighborhood_health), x = 0.5, xanchor = "center"),
  ...
  output$healthDensityChart <- renderPlotly({
    ...
    plot_ly(data = health_data, x = ~Category, y = ~as.numeric(Value), type = 'bar', color = ~Category, colors = "Paired", 
      ...
      add_markers(x = ~Category, y = ~CountyAvg, name = "Avg Across Counties",
                  ...
                  hoverinfo = "text",
                  showlegend = FALSE) %>%
      layout(title = list(text = "Medical Professionals", font = list(size = 14), x = 0.5, xanchor = "center"),
             xaxis = list(title = list(text = "Professionals", font = list(size = 12)), showticklabels = FALSE),
  ...
  output$healthInsuranceChart <- renderPlotly({
    ...
    plot_ly(data = insurance_data, x = ~Category, y = ~as.numeric(Value), type = 'bar', color = ~Category, colors = "Dark2",
      ...
      add_markers(x = ~Category, y = ~CountyAvg, name = "Avg Across Counties",
                  ...
                  hoverinfo = "text",
                  showlegend = FALSE) %>%
      layout(title = list(text = "Health Insurance", font = list(size = 14), x = 0.5, xanchor = "center"),
...
tab5d_server <- function(input, output, session) {
...
ui <- fluidPage(
  ...
  tags$head(
    tags$title("Illinois Community Health Dashboard"),
    tags$script(async = NA, src = "https://www.googletagmanager.com/gtag/js?id=G-N806Q4JMB1"),
    tags$script(HTML("
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());
    gtag('config', 'G-N806Q4JMB1');
    ")),
  ...
  div(id = "landing_page",
      ...
      img(src = "logo.png", height = "80px", style = "margin-bottom: 20px;"),
